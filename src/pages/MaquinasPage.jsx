...
const MaquinasPage = () => {
  ...
  // Descobre usuÃ¡rio atual (opcional, sÃ³ para esconder UI de gestor)
  ...
  const excluir = async () => {
    ...
    try {
      ...
      if (status === 401) msg = 'UsuÃ¡rio nÃ£o autenticado. FaÃ§a login e tente novamente.';

      else if (status === 403) msg = t('maquinas.toasts.deleteForbidden') || 'PermissÃ£o negada (somente gestor).';

      else if (status === 404) msg = t('maquinas.toasts.deleteNotFound') || 'MÃ¡quina nÃ£o encontrada (tente atualizar).';

      else if (status === 409) msg = t('maquinas.toasts.deleteBlocked') || 'MÃ¡quina com vÃ­nculos.';
            ...
            <div className={styles.grid}>
              ...
              {maquinasComStatus.map((maquina) => (
                ...
                <div key={maquina.id} className={`${styles.card} ${getStatusClass(maquina.statusDestaque)}`}>
                  ...
                  {/* botÃ£o 3 pontinhos (sÃ³ para gestor) */}

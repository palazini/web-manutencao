...
import styles from './EditarChecklistDiarioPage.module.css'; // Usaremos um CSS próprio

const EditarChecklistDiarioPage = () => {
  ...
  useEffect(() => {
    const fetchChecklist = async () => {
      const docRef = doc(db, 'dailyChecklistTemplates', id);
      ...
      if (docSnap.exists()) {
        ...
        navigate('/checklists-colaboradores');
      }
      setLoading(false);
    };
    fetchChecklist();
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      ...
      navigate('/checklists-colaboradores'); // Volta para a lista
    } catch (error) {
  ...
  if (loading) return <p style={{padding: '20px'}}>Carregando checklist...</p>;

  return (
    <>
      <header style={{ padding: '20px', backgroundColor: '#ffffff', borderBottom: '1px solid #e0e0e0' }}>
        <h1>Editar Checklist Diário</h1>
      </header>
...
export default EditarChecklistDiarioPage;
